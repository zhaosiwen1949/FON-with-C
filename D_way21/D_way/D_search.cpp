...
void D_search::Search_way(int n,int start,int cost[21][21])
{
	int i,j;
	int s[21];//定义具有最短路径的节点子集s
	//初始化最小路径代价和前一跳节点值------------------------------------------//
	for (i = 0; i < n; i++)
	{
    ...
    */
	...
	else
	{
		...
		for (j = count-1; j >= 1; j--)
		{
			...
			{
				//printf("the %d channnel hit is not the same as the 1st hit! block ++ \n",j);
				block=1;
				return block;			
			}
			else
			{
			//	printf("the %d channnel hit is as same as the 1st hit! ->\n",j);
